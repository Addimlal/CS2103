...
Absyn *newProcDec(int line, Sym * name, Absyn * params, Absyn * decls, Absyn * body)
{
	Absyn *node;
...
static void showOpExp(Absyn * node, int n)
{
	...
	case ABSYN_OP_EQU:		say("EQU"); break;
	case ABSYN_OP_NEQ:		say("NEQ"); break;
	case ABSYN_OP_LST:		say("LST"); break;
	case ABSYN_OP_LSE:		say("LSE"); break;
	case ABSYN_OP_GRT:		say("GRT"); break;
	case ABSYN_OP_GRE:		say("GRE"); break;
	case ABSYN_OP_ADD:		say("ADD"); break;
	case ABSYN_OP_SUB:		say("SUB"); break;
	case ABSYN_OP_MUL:		say("MUL"); break;
	case ABSYN_OP_DIV:		say("DIV"); break;
	default:
...
static void showNode(Absyn * node, int indent)
{
	...
	switch (node->type) {
	case ABSYN_NAMETY:		showNameTy(node, indent); break;
	case ABSYN_ARRAYTY:		showArrayTy(node, indent); break;
	case ABSYN_TYPEDEC:		showTypeDec(node, indent); break;
	case ABSYN_PROCDEC:		showProcDec(node, indent); break;
	case ABSYN_PARDEC:		showParDec(node, indent); break;
	case ABSYN_VARDEC:		showVarDec(node, indent); break;
	case ABSYN_EMPTYSTM:		showEmptyStm(node, indent); break;
	case ABSYN_COMPSTM:		showCompStm(node, indent); break;
	case ABSYN_ASSIGNSTM:		showAssignStm(node, indent); break;
	case ABSYN_IFSTM:		showIfStm(node, indent); break;
	case ABSYN_WHILESTM:		showWhileStm(node, indent); break;
	case ABSYN_CALLSTM:		showCallStm(node, indent); break;
	case ABSYN_OPEXP:		showOpExp(node, indent); break;
	case ABSYN_VAREXP:		showVarExp(node, indent); break;
	case ABSYN_INTEXP:		showIntExp(node, indent); break;
	case ABSYN_SIMPLEVAR:		showSimpleVar(node, indent); break;
	case ABSYN_ARRAYVAR:		showArrayVar(node, indent); break;
	case ABSYN_DECLIST:		showDecList(node, indent); break;
	case ABSYN_STMLIST:		showStmList(node, indent); break;
	case ABSYN_EXPLIST:		showExpList(node, indent); break;
	default:
